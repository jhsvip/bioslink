...
class BIOSManager:
    ...
    def edit_selected(self):
        ...
        nt = scrolledtext.ScrolledText(frm,width=40,height=4) # 修复了这里
        ...
        def save():
            ...
            self.bios_data[idx] = { 
                ...
                "model":mo.get(),
                "link":li.get(),
                "notes":nt.get("1.0","end-1c") 
            }
            self.save_data()
            self.update_bios_list() # 刷新列表以保持视图一致
            win.destroy()
        ...
        try:
            if path.endswith(".json"):
                with open(path, 'w', encoding='utf-8') as f:
                    json.dump(self.bios_data, f, indent=2, ensure_ascii=False)
                messagebox.showinfo("导出成功", f"JSON文件已保存至：{path}")
            else:
                with open(path, 'w', encoding='utf-8') as f:
                    for item in self.bios_data:
                        f.write(f"{item['brand']} - {item['model']} - {item['link']}\n")
                messagebox.showinfo("导出成功", f"文本文件已保存至：{path}")
        except Exception as e:
